...
public class AndroidXMLReader {
	...
	public <T> Object read(URI uri, Class<T> classType, UserCredentials credentials) 
		...
		try {
			...
			resources = serializer.read(classType, is, /*is1,*/ false);
		} catch(Exception e){
			e.printStackTrace();
		} finally {
	...
	public <T> Object read(URI uri, Class<T> classType) throws NetworkConnectionException {
		return read(uri, classType, null);
	}

	public ServerResources readServerResources(URI uri) throws NetworkConnectionException {
